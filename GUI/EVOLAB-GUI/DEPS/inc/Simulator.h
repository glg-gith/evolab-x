...
class Simulator
{
    ...
    public:
        ...
        Simulator(LabNetwork* net) { this->net = net; this->time = 0; this->mtrand = net->get_rng(); };

        void set_network( LabNetwork* net ) { this->net = net; this->mtrand = net->get_rng(); };
        LabNetwork* network() { return(net); };
        ...
        void set_all_nodes_to_state ( stateType s ) {
            vector<LabNode*> nodes = net->get_nodes();
            set_these_nodes_to_state(nodes, s);
        };

        void set_these_nodes_to_state (vector<LabNode*> nodes, stateType s) {
            for (unsigned int i = 0; i < nodes.size(); i++) nodes[i]->set_state(s);
        }

        // choose n nodes without replacement
        vector<LabNode*> rand_choose_nodes (int n) {
            assert(n > -1 and n <= net->size());
            vector<LabNode*> nodes = net->get_nodes();
            vector<LabNode*> sample(n);
        ...
        vector<LabNode*> rand_set_nodes_to_state (int n, stateType state) {
            vector<LabNode*> sample = rand_choose_nodes(n);
        ...
        virtual vector<LabNode*> rand_infect(int) {vector<LabNode*>x; return x;};
                                 // cumulative infected
...
class Derived_Example_Simulator: public Simulator {
    ...
    public:
        ...
        Derived_Example_Simulator(LabNetwork* net);
        ~Derived_Example_Simulator();
