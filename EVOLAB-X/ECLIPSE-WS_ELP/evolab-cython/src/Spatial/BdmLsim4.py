...
# Reference to numpy pytohn module (a math module used here
# to manipulate easily arrays and matrixes)
import numpy as np
...
nb_cols     = 100 #10;//           # Number of columns for display

p_init      = 0.5 #0.01
tt_out      = 1                    # Output every 1 unit of time
tt_out_st   = 5
...
# Fills an array of size nb_sites with zeros
arr_mn = np.zeros(nb_sites, dtype=float)

# Calculator (instance of PyLabSiteSetsCalculator)
_C = None
...
# Indices of the possible attributes
...
#    print "BIRTH fired !"
    ...
    if (bs_data.GetState() == EMPTY):
        ...
        bs_data.Copy(s_data, cyelp.IND_DEP);
...
#    print "MOVEMENT fired !"
    ...
    if (ms_data.GetState() == EMPTY):
        ...
        ms_data.Copy(s_data, cyelp.IND_DEP);
...
#    print "MUTATION fired !"
    ...
    # Rates update (static (C++) = @classmethod (Python))
    BdmLsim4.UpdateRatesLocal(node)
...
The class "PyLabSimulatorTimeNet" inherits from "PyLabSimulatorTime" + handles an object of type 
"PyLabNetBinding" for dealing with the network (See : PyLabSimulatorTimeNet.netBinding).
'''
class BdmLsim4(PyLabSimulatorTimeNet):
    ...
    def CheckInputs(self):
        ...
        if (not ok): print("The given d, b, mu, mo, x1, x2 or L value is not in range [0.0, 1.0]")
        return ok
    ...
    def RealizeEvent(self):
        ...
        # Method 1 : The way used in C++ versions
        # Note : as iterating through large loops is very time consuming
        #        in Python, will prefer to delegate this task to a ready made 
        #        embedded C++ function (Method 2) : 
        #        LabRng::GetNextIntMultinomial(double max_cumsum, vector<double> p_nomes)
        
#        length = len(nodes)
#        n = 0
#        for i in xrange(length):
#            add += arr_mn[i] #nodes[i].GetUserData().GetEventStackRate()
#            if (p1 < add): 
#                break
#            else:
#                n += 1
        ...
        nodes[n].GetUserData().RealizeEvent(p1, self.GetTime()); #mettre un cerr dans RealizeEvent
...
#                # END IT2
                ...
                for i in xrange(node.GetNeighborsCount()):
                    ngh = node.GetNeighborAt(i)
                    if (ngh.GetUserData().GetState() != EMPTY):
                        nox += 1
                        for j in xrange(ngh.GetNeighborsCount()):
    ...
    def InitPopulation(self):
        ...
        print "Before models _____________________________"
            
        #vector<LabSiteBase *> model_sites;
        model_sites = []
        for i in xrange(2):
            a_site = PyLabSiteEvent(self);
            ...
            a_site.PushEvent("NONE", 0, None, rate_callback, realize_callback);
    
            rate_callback = PyLabCallBack(BirthRate)
            realize_callback = PyLabCallBack(RealizeBirth)
            a_site.PushEvent("BIRTH", 0, None, rate_callback, realize_callback);
    
            rate_callback = PyLabCallBack(DeathRate)
            realize_callback = PyLabCallBack(RealizeDeath)
            a_site.PushEvent("DEATH", 0, None, rate_callback, realize_callback);
    
            rate_callback = PyLabCallBack(MovementRate)
            realize_callback = PyLabCallBack(RealizeMovement)
            a_site.PushEvent("MOVEMENT", 0, None, rate_callback, realize_callback);
    
            rate_callback = PyLabCallBack(MutationRate)
            realize_callback = PyLabCallBack(RealizeMutation)
            a_site.PushEvent("MUTATION", 0, None, rate_callback, realize_callback);
    
            model_sites.append(a_site);
            
        print "After models _____________________________"

        # Cumsum of nomes must be 1.0 !!
        dist = [p_init, 1 - p_init]
        self.netBinding.Populate1(dist, model_sites, True)
#        # Cumsum of nomes must be equal to network's size (= population max size = nb_sites) !!
#        dist = [52000, 48000]
#        self.netBinding.Populate2(dist, model_sites, True)

        print "After Populate1 _____________________________"
        ...
        it = iter(nodes)
        for item in it:
            if ( item.GetUserData().GetState() == EMPTY  ):
                empty += 1
            else:
                occupied += 1

        print "OCCUPATION START >> ", empty, " | ", occupied
    
        universe = _C.CNT(_C.GS("U"))
        empty = _C.CNT(_C.GS("EMPTY"))
        unset = _C.CNT(_C.GS("UNSET"))
        occupied = _C.CNT(_C.GS("U") - _C.GS("EMPTY"))
    
        print "OCCUPATION >> ", universe, " | ", empty, " | ", occupied, " | ", unset
...
#        self.DumpLattice()
        ...
        empty = _C.CNT(_C.GS("EMPTY"));
        occupied = _C.GS("RESIDENT").CU(_C.GS("MUTANT"));
    
        print "OCCUPATION >> ", empty, " | ", occupied, " ( ", _C.GS("RESIDENT").Cardinality(), " ", _C.GS("MUTANT").Cardinality(), " )"
        attrs = [ X, M ]
        deps =  [ cyelp.IND_DEP, cyelp.IND_DEP ]
        avgs = _C.MAVG( (_C.GS("RESIDENT") + _C.GS("MUTANT")), deps, attrs )
        print "MEAN X     >> ", avgs[0]
        print "MEAN M     >> ", avgs[1]
...
def main():
    ...
    ok = sim.CheckInputs()
    # If wrong input detected : leave the program with error code '-1'
    if (not ok): return -1
