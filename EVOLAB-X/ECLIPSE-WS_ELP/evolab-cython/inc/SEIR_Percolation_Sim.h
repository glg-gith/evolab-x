...
class SEIR_Percolation_Sim: public Percolation_Sim
{
    ...
    public:
       ...
       // vector<LabNode*> infected;
       // vector<LabNode*> recovered;
        vector<LabNode*> exposed;

        SEIR_Percolation_Sim():Percolation_Sim() {};
        SEIR_Percolation_Sim(LabNetwork* net):Percolation_Sim(net) {};

        void rand_expose (int n) {
            assert(n > 0);
            vector<LabNode*> patients_zero = rand_set_nodes_to_state(n, E);
            for (int i = 0; i < n; i++) {
        ...
        void step_simulation () {
            ...
            print_states();
            vector<LabNode*> new_exposed;
            for (int i = 0; i < infected.size(); i++) {
                LabNode* inode = infected[i];
                vector<LabNode*> neighbors = inode->get_neighbors();
                for (int j = 0; j < neighbors.size(); j++) {
                    LabNode* test = neighbors[j];
