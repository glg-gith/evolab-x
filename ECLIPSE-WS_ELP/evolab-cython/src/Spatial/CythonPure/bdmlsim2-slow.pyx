...
def RealizeBirth(node):
    ...
    cdef int birth_site, birth_neighbour
...
class BdmLsim2(PyLabSimulatorTimeNet):
    ...
    def RealizeEvent(self):
        ...
        nodes = self.netBinding.GetNetwork().get_nodes()
        cdef np.ndarray[dtype=object, ndim=1] np_nodes = self.netBinding.GetNetwork().get_nodes()
        cdef unsigned int length = len(nodes)
        cdef unsigned int n = 0
        for i in range(length):
            add += np_nodes[i].GetUserData().GetEventStackRate()
            if (p1 < add): 
                break
            else:
                n += 1
        ...
        np_nodes[n].GetUserData().RealizeEvent(p1, self.GetTime());
...
#        global nb_sites, con_deg
        ...
        if (sim.GetDoStartFromDump()):
            ...
            dist = [0.0 for i in range(con_deg+1)]
            dist[con_deg] = 1.0
