...
////	return clearMOY_I * (_C->GS("T0").Cardinality())  +  clearMOY_T * (_C->GS("T1").Cardinality());
	return clearMOY * (_C->GS("I").Cardinality());
}
...
//	cout << "FIRE DEATH !" << endl;
	...
	int i;
	double natural_death = (mu * _C->GS("S").Cardinality());
	double max = natural_death + ((mu + virMOY_I) * _C->GS("T0").Cardinality()) + ((mu + virMOY_T) * _C->GS("T1").Cardinality());
...
//		if (p1 < (fgeno)) { geno = i; break; };
	...
	LabSiteBase *site = _C->GI(_C->GS("S").GetFirst());
	if (site) {
...
//	cout << "FIRE CLEARANCE !" << endl;
	...
	double max = (clearMOY * _C->GS("I").Cardinality());
...
void DrugRes::Update_F_Averages()
{
	...
	double nb_UT = (double)(_C->GS("T0").Cardinality());
...
//			clearMOY_I += (clear[i]*gI[i]) / nb_I;
			virMOY_I   += (VTCmtx[DrugRes::alpha][i][0] * I_UT) / nb_UT;
			betaMOY_I  += (VTCmtx[DrugRes::beta][i][0]  * I_UT) / nb_UT;
			clearMOY_I += (VTCmtx[DrugRes::gamma][i][0] * I_UT) / nb_UT;
		}
		if (nb_T != 0)
		{
...
void DrugRes::Census()
{
	...
	stringstream ss;
	ss << "time :" << tt << " | " << eventStack->GetStats() << " > ";
	ss << _C->GS("S").Cardinality() << " ( " 	<< _C->GS("g0").Cardinality() << ", "
	...
	stringstream ss2;
	ss2 << "[";
	for (int i=0; i< eventStack->GetSize(); i++)
		ss2 << " " << eventStack->GetEventRate(i);
	ss2 << " ] ";
	ss2 << betaMOY << "\n";
	outputer->InsertText(ss2.str());
}
...
void DrugRes::AfterStep() {
	// Check pathogen extinction
	stringstream ss;
	if (_C->GS("I").Cardinality() == 0) {
		ss << GetTime();
		cerr << "Pathogen extinction at t=" + ss.str() << endl;
		exit(0);
	}
}
