...
Cœur partagé du modèle **DrugRes** (S-I-S évolutif de Sylvain Gandon).

Traduction fidèle de :
`evolab-x/Commented-Sims/Sources/C++/Drugres/DrugRes.cpp`

Événements : BIRTH, DEATH, INFECTION, CLEARANCE, SUPERINFECTION, MUTATION.
Le pathogène a 4 génotypes (2 loci) : 0=ab, 1=Ab, 2=aB, 3=AB. Chaque infecté
porte aussi un statut `treated` (0/1). Les traits virulence / transmission /
clairance dépendent du génotype et du traitement via la matrice VTC.

Les génotypes sont gardés en **0-based** (0..3) pour préserver à l'identique la
logique de masques de bits de la recombinaison (cf. C++).
"""
module DrugResModel
