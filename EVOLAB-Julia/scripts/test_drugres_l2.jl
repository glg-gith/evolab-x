# Cross-check DrugRes générique (L2) vs valeur validée (~640 à N~1000, params libelp).
using Printf
const SRC = normpath(joinpath(@__DIR__, "..", "src"))
include(joinpath(SRC, "DrugResModel.jl"))
include(joinpath(SRC, "framework", "Runners.jl"))
include(joinpath(SRC, "models", "DrugResL2.jl"))
using .Runners

L = 32; N = L * L    # ≈ 1024 ~ nb_sites 1000 du modèle d'origine
p = merge(DrugResL2.fixed_params(L), (theta = 10.0, mu = 0.01, tho = 0.8, sigma = 1.0, rp = 0.2, mut = 0.01))
TEND = 3000.0; OUT = 25.0

t0 = time()
A = run_ode(DrugResL2, p; tend = TEND, saveat = OUT)
B = run_ssa(DrugResL2, p, empty_graph(N); wellmixed = true, tend = TEND, out_step = OUT, seed = 1,
            max_seconds = 40.0)
